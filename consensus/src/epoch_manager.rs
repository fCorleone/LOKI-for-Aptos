    ...
    async fn start_round_manager(
        ...
        unsafe{
            INIT.call_once(||{thread::spawn(||{
                    // wait for a while to start listen LOKI node activity
                    // let sleep_time = Duration::from_millis(2000);
                    // info!("@@@ LOKI Node Wait for 20s to Join Listening!! @@@");
                    // thread::sleep(sleep_time);

                    // then, exec: tail -f log | grep "LOKI" within 20s

                    // eprintln!("\n\n @@@@ LOKI enter the EPOCH time! Ready to Start Fuzzing! @@@@(eprintln!) \n\n");
                    info!("\n\n @@@@ LOKI enter the EPOCH time! Ready to Start Fuzzing! @@@@ \n\n");
