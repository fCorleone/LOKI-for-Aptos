...
use rand::seq::IteratorRandom; // new
...
impl NetworkSender {
    ...
    //     info!("LOKI in `send_to_others` Function!");
    //     // Directly send the message to ourself without going through network.
    ...
    pub async fn send_to_others(&self, msg: ConsensusMsg) {
        ...
        let mut rng = rand::thread_rng();
        // let temp : u8 = rng.gen();
        let selected_validators: Vec<_> = other_validators
            .iter()
        ...
        // Broadcast message over direct-send to the selected validators.
        if let Err(err) = self
            .consensus_network_client
            .send_to_many(selected_validators, msg) {
            warn!(error = ?err, "Error broadcasting message to selected validators");
        }
    }


    /// added by iceray, this is used to send packages to some random nodes for crusader
    pub async fn send_to_others_all(&self, msg: ConsensusMsg) {
        // info!("This is the LOKI fuzzer functions!!");
        info!("LOKI in `send_to_others_all` Function!");
        // Directly send the message to ourself without going through network.
        ...
        info!(
            "LOKI fuzzer randomly send to other ALL validator nodes: {:?}",
            other_validators
        );
