...
impl RoundManager {
        ...
        if is_current_proposer {
            ...
            tokio::spawn(async move {
                if let Err(e) = monitor!(
                    ...
                    Self::generate_and_send_proposal(  /* Passive Mutate Process */
                        epoch_state,
                        new_round_event,
                        network,
                        sync_info,
    ...
    async fn generate_and_send_proposal(
        ...
        info!("@@$$ LOKI enter generate_and_send_proposal");
        ...
        // loki mutate the proposal messages and send them to others
        info!("$$ LOKI is about to mutate the proposal messages for author: {:?}", loki_author);
        info!("$$ LOKI in ROUND MANAGER in mutate_proposal ...");

        // network.send_to_others(ConsensusMsg::ProposalMsg(Box::new(loki_proposal_msg))).await;

        info!("@*@ LOKI: Passive 0_mutate_proposal start");
        // Passive Mutate
        let mut loki_proposal_msg = mutate_proposal(proposal_msg.clone(), loki_safety_rules, loki_author);
        info!("LOKI has mutated the proposal message: {:?}", loki_proposal_msg);

        network.send_to_others_all(ConsensusMsg::ProposalMsg(Box::new(loki_proposal_msg))).await;

        info!("@*@ LOKI: Passive 0_mutate_proposal end");
    ...
    pub async fn process_local_timeout(&mut self, round: Round) -> anyhow::Result<()> {
        ...
        if self.local_config.enable_round_timeout_msg {
            ...
            // LOKI mutates the vote messages and sends them out
            info!(
                "LOKI is mutating a vote message with timeout: {:?} and sync info: {:?}",
                timeout_vote,
                self.block_store.sync_info()
            );
            info!("@*@ LOKI: Passive 0_mutate_vote start");
            let loki_timeout_vote_msg = mutate_vote(VoteMsg::new(timeout_vote, self.block_store.sync_info()));
            info!(
                "LOKI has mutated the vote message: {:?}. Sending to others.",
                loki_timeout_vote_msg
            );

            self.network.send_to_others(ConsensusMsg::VoteMsg(Box::new(loki_timeout_vote_msg)));
            info!("LOKI has sent the mutated vote message to other validators.");

            info!("@*@ LOKI: Passive 0_mutate_vote end");
    ...
    pub async fn init(&mut self, last_vote_sent: Option<Vote>) {
        info!("LOKI in init RoundManager");
        let epoch_state = self.epoch_state.clone();
