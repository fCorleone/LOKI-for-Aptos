...
pub fn start_fuzzer(network_ : NetworkSender, LAST_MSG : &mut Vec<ConsensusMsg>, safety_rules:Arc<Mutex<MetricsSafetyRules>>,author:Author){
    ...
    // let mut _time: u32 = 0;
    info!(
        "LOKI fuzzer has started!!! (active)"
    );
    ...
    loop{
        if !LAST_MSG.is_empty(){
            ...
            match chosen_msg{
                ConsensusMsg::ProposalMsg(m) => {
                    ...
                    info!("$$ LOKI: enter ACTIVE FUZZING mutate_proposal ...");
                    info!("@*@ LOKI: 1_mutate_proposal start");
                    ...
                    info!("@*@ LOKI: 1_mutate_proposal end");
                }
        ...
        // sleep for 1 second ---> 100
        let sleep_time = time::Duration::from_millis(100);
        info!("@@@ LOKI Node sleep for 1s! @@@");
...
pub fn mutate_proposal(
    ...
    match temp_rd % 6 {
        0 => {
            ...
            info!("*@* LOKI: mutate_round 1");
        }
        1 if round > 10 => {
            round -= 10;
            info!("@@ LOKI: Round -= 10, new round: {:?}", round);
            info!("*@* LOKI: mutate_round 2");
        }
        2 => {
            round = temp_rd;
            info!("@@ LOKI: Round completely in Random Value: {:?}", round);
            info!("*@* LOKI: mutate_round 3");
        }
        3 => {
            // round = 0;
        ...
        4 => {
            ...
            info!("*@* LOKI: mutate_round 5");
        }
        5 => {
            // skipping
            info!("@@ LOKI: Round SKIPPING: {:?}", round);
            info!("*@* LOKI: mutate_round 0");
        }
        _ => {}
    }
    ...
    // 检测sig的前半部分，也就是 safety_rules.lock().sign_proposal(&block_data) 的部分，如果没有返回 Err ，则继续unwrap进行解包；如果返回 Err，则进行info！

    /* 可能存在 Panic：试图从 Result 中进行解包，但如果当前的 Result 是一个 Err 就会产生 Panic */
    /* 这是为什么会产生这种情况呢？ */
    /* 错误的核心是一个关于共识轮次（PreferredRound）的问题，程序期望得到 (40, 638) 这样的轮次值，但实际上得到了不符合预期的值。通过改进错误处理和检查轮次 */
    /* 问题的关键就是在于： sign_proposal 中，会出现返回 Err 的情况！*/
    /* 这是一个位于 metrics_safety_rules.rs 中的方法 */
    // 新的 block_data 是用新的 round 和 timestamp 进行的，
...
fn mutate_payload(mut payload: Payload) -> Payload {
    match &mut payload {
        Payload::DirectMempool(txns) => {
            if txns.is_empty() {
                info!("*@*@* LOKI: payload: directMempool empty");
                return payload;
            }
            info!("@@@@@ LOKI: payload MUTATE DirectMempool!");
            info!("*@*@* LOKI: payload: directMempool start");
            ...
            info!("*@*@* LOKI: payload: directMempool end");
        ...
        Payload::QuorumStoreInlineHybrid(inline_batches, proof_with_data, maybe_limit) => {
            if inline_batches.is_empty() {
                ...
                info!("*@*@* LOKI: payload: quorumStoreInlineHybrid empty; mutate_proof_single_field start");
                ...
                info!("*@*@* LOKI: payload: quorumStoreInlineHybrid not empty, start");
                let mut rng = rand::thread_rng();
                ...
                if txns.is_empty() {
                    ...
                    info!("*@*@* LOKI: payload: quorumStoreInlineHybrid not empty, but txn empty; mutate_proof_single_field start");
...
fn mutate_proof_with_data_single_field(proof_with_data: &mut ProofWithData) {
    ...
    if proof_with_data.proofs.is_empty() {
        ...
        info!("@*@*@ LOKI: mutate_proof_single_field end, proof_data is empty");
        return;
    }
    info!("@@@@@ LOKI: Mutating proof_with_data!");
    info!("@*@*@ LOKI: mutate_proof_single_field start");
    ...
    if !proof_bytes.is_empty() {
        ...
        if let Ok(mutated_proof) = bcs::from_bytes::<ProofOfStore>(&proof_bytes) {
            ...
            info!("@*@*@ LOKI: mutate_proof_single_field success");
        } else {
            info!("@@@@@ LOKI: Mutation resulted in invalid ProofOfStore!");
            info!("@*@*@ LOKI: mutate_proof_single_field invalid");
        }
    } else {
        info!("@*@*@ LOKI: mutate_proof_single_field proof_bytes is empty");
    }
}
...
fn mutate_blockInfo(cur_info: &BlockInfo) -> BlockInfo{
    ...
    if temp4 % 4 == 0{
        ...
        // info!("Mutating VOTE! mutate_blockINFO make timestamp-- ==> panic");
        timestamp_usecs -= temp4 % 10000;
    }
    else if temp4 % 4 ==2{
